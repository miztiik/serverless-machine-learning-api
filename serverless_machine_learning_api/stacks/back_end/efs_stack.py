...
class EfsStack(core.Stack):
    ...
    def __init__(self,
        ...
        # Create Lambda Security Group to connect to EFS
        self.lambda_efs_sg = _ec2.SecurityGroup(
            self,
            id="lambdaToEfsSecurityGroup",
            vpc=vpc,
            security_group_name=f"lambda_to_efs_sg_{id}",
            description="Lambda Security Group to connect to EFS"
        )

        self.lambda_efs_sg.add_ingress_rule(
            peer=_ec2.Peer.ipv4(vpc.vpc_cidr_block),
            connection=_ec2.Port.tcp(2049),
            description="Allow EC2 instances within the same VPC to connect to EFS"
        )
        ...
        self.efs_ap_ml = _efs.AccessPoint(
            self,
            "efsMlAccessPoint",
            path="/ml",
            file_system=self.efs_share,
